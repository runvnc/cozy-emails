...
var Account, AccountConfigError, BadRequest, MSGBYPAGE, Mailbox, Message, NotFound, _, async, contentToBuffer, log, messageUtils, multiparty, querystring, ref, stream_to_buffer_array,
...
module.exports.details = function(req, res, next) {
  return res.send(200, req.message.toClientObject());
};
...
module.exports.parseSendForm = function(req, res, next) {
  ...
  form.on('part', function(part) {
    stream_to_buffer_array(part, function(err, bufs) {
      if (err) {
        return nextonce(err);
      }
...
contentToBuffer = function(req, attachment, callback) {
  ...
  if (attachment.url) {
    stream = req.message.getBinary(filename, function(err) {
      if (err) {
...
module.exports.send = function(req, res, next) {
  var account, destination, draftBox, files, jdbMessage, message, previousUID, ref1, sentBox, steps, uidInDest;
  log.debug("send");
  message = req.body;
  account = req.account;
  files = req.files;
  ...
  if (previousUID || message.isDraft) {
    steps.push(function(cb) {
      var id;
  ...
  return async.series(steps, function(err) {
    ...
    return res.send(200, jdbMessage.toClientObject());
  });
};
...
module.exports.conversationGet = function(req, res, next) {
  return res.send(200, req.conversation.map(function(msg) {
    return msg.toClientObject();
  }));
};
